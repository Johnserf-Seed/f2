...
- 文件名规则变化，升级后会按新文件名重新下载一次（#248）：文案与昵称不再把标点、空格、日文假名、emoji 等字符替换为下划线，只把系统不允许的 `\ / : * ? " < > |` 换成外观相近的全角字符，换行等控制字符换成空格或去掉，并去掉首尾空格与结尾的点。多数作品的文件名和部分作者的文件夹名会因此变化，同步主页时这些作品会按新名称重新下载，旧文件仍留在原处，可按需清理。各应用的命名模板文档已同步说明。
