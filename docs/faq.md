...
当使用代理时出现连接失败的错误，请检查以下问题：

::: details :link: 解决办法
1. **代理类型是否正确**：确保选择了正确的代理类型（http/https/socks4/socks5）
2. **代理地址和端口**：检查代理服务器地址和端口是否正确
3. **认证信息**：如果代理需要认证，确保用户名和密码正确
4. **网络连通性**：测试代理服务器是否能正常访问
5. **防火墙设置**：检查防火墙是否阻止了代理连接
