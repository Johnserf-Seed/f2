...
> - 为了增强安全性并降低碰撞概率，`F2` 每次推送都会自动生成由字母和数字组成的随机 `iv`（`CBC` 为 16 位，`GCM` 为 12 位）并随请求发送。在 `Bark` 中 `iv` 可以任意填写。
...
> - 不建议使用 `ECB` 模式：相同的明文块会得到相同的密文，容易暴露推送内容的规律。`F2` 保留 `ECB` 只为兼容 `Bark` 中的同名选项，使用时会输出安全警告。
