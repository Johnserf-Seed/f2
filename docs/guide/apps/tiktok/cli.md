...
::: tip :bulb: 提示
作品按 `<path>/tiktok/<mode>/<用户名>` 分目录保存，用户名即 `uniqueId`（`@` 后面的部分）。作者修改用户名后，再次下载该作者（任一模式）时会把各下载模式下旧用户名的目录一起重命名为新用户名，已下载的作品随目录保留，文件名没有变化的作品不会重新下载；之后再改名也会继续跟随。某个模式下新用户名的目录已经存在时，该模式的两个目录都保持不变并在日志中提示，手动合并并删除旧目录后不再提示。
:::
...
::: tip :bulb: 提示
文件名中的文案与昵称会原样保留标点、空格和各国文字，只把系统不允许的字符 `\ / : * ? " < > |` 换成外观相近的全角字符（例如 `?` 换成 `？`），换行等控制字符会换成空格或去掉；文案超过 200 字节时会截断中间部分，整个文件名连同后缀超过 255 字节时也会截断中间部分，这样也能保存到 NAS 等按字节限制文件名长度的位置；Windows 下路径超过 260 个字符时会自动改用长路径，不需要修改系统设置。完整的原始文案可以用 `--desc` 保存到 `_desc.txt` 文件。
:::
...
下载日期区间发布的作品，格式：`年-月-日|年-月-日`。例如：`2022-01-01|2023-01-01`，设置`all` 为下载所有作品。首尾两天都包含在内，按北京时间计算；格式错误或结束日期早于开始日期时直接报错退出，不会发起请求。
...
::: code-group
```bash[SOCKS5代理]
f2 tk --proxies socks5 127.0.0.1:1080
```

```bash[HTTP代理]
f2 tk --proxies http proxy.example.com:8080
```

```bash[带认证的代理]
# 可在配置文件中设置用户名密码
f2 tk --proxies socks5 user:pass@127.0.0.1:1080
```
:::

> [!IMPORTANT] 重要提示
> - **SOCKS代理推荐**：对于TikTok等平台，推荐使用 SOCKS5 代理以获得更好的兼容性
