...
::: tip :bulb: 提示
作品按 `<path>/tiktok/<mode>/<用户名>` 分目录保存，用户名即 `uniqueId`（`@` 后面的部分）。作者修改用户名后，再次下载时会把旧用户名的目录重命名为新用户名，已下载的作品随目录保留，文件名没有变化的作品不会重新下载；各下载模式的目录在以该模式下载时分别重命名。新用户名的目录已经存在时两个目录都保持不变并在日志中提示，需要时可手动合并。
:::
