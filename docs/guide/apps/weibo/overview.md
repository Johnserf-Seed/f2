...
用于创建或重命名用户目录。本地记录中的昵称（`local_user_data["nickname"]`）与当前昵称不同时，把旧昵称的用户目录重命名为当前昵称，已下载的文件随目录保留；没有本地记录或昵称没有变化时直接创建用户目录。
...
::: tip :information_source: 提示
- 旧昵称与新昵称的目录都按 `create_user_folder` 的规则计算（`path`、应用名、`mode`），只处理当前下载模式下的目录。
- 旧昵称的目录不存在时直接创建新目录；新昵称的目录已存在时两个目录都保持不变，不会覆盖或合并，并在日志中提示。
- 重命名失败（例如目录中的文件正被其他程序占用）时本次继续使用旧目录，下次运行再尝试。
- 该接口集成在 `handler` 的 `get_or_add_user_data` 中，开发者无需关心，直接调用 `handler` 的数据接口即可。
:::
